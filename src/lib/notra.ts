...
export async function getGeoDashboard(projectId: string, days: number): Promise<GeoDashboardData> {
  ...
  const results = await Promise.allSettled([
    ...
    notraRequest<GeoScansResponse>(`${projectPath}/scans?limit=50&page=1`),
  ...
  return {
    ...
    scans: valueOr("Scans", scansResult, {
      organization,
      pagination: { currentPage: 1, limit: 50, nextPage: null, previousPage: null, totalItems: 0, totalPages: 1 },
      scans: [],
    }),
